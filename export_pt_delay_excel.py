...
def main() -> None:
    ...
    # Load baselines per sim index
    baselines = {}
    for sim in range(1, args.sims + 1):
        p = os.path.join(simdir, f"stop_events_baseline_{sim}.xml")
        if os.path.exists(p):
            ...
            print(f"Warning: missing baseline file {p}")
    ...
    with pd.ExcelWriter(args.out, engine="openpyxl") as writer:
        for value in sorted(value_to_simpaths.keys()):
            rows = []
            for sim, path in sorted(value_to_simpaths[value].items()):
            ...
            s = df["delay_delta_s"].describe(percentiles=[0.5, 0.9]).to_dict()
            summaries.append({
                "value": value,
